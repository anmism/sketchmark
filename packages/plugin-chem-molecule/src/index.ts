...
type ChemCommandType = "atom" | "bond" | "ring" | "label";
type BondOrder = 1 | 2 | 3 | "alternating";
type LabelSide = "top" | "right" | "bottom" | "left" | "center";

interface ChemCommand {
  type: ChemCommandType;
  id: string;
...
type ChemEntity = AtomEntity | RingEntity;
...
const SUPPORTED_COMMANDS = new Set<ChemCommandType>([
  "atom",
  "bond",
  "ring",
  "label",
...
const DEFAULTS: Required<ChemMoleculePluginOptions> = {
...
export function chemMolecule(
  options: ChemMoleculePluginOptions = {},
): SketchmarkPlugin {
  return {
    name: "chem-molecule",
    preprocess(source) {
...
export function compileChemMolecule(
  source: string,
  options: ChemMoleculePluginOptions = {},
): string {
  const settings = { ...DEFAULTS, ...options };
  const lines = source.split(/\r?\n/);
  const commandByLine = new Map<number, ChemCommand>();
  const entities = new Map<string, ChemEntity>();
  let hasChem = false;
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (inTripleQuoteBlock || !trimmed.startsWith("chem.")) continue;
...
function parseChemCommand(line: string, lineNumber: number): ChemCommand {
  ...
  const type = commandToken.slice("chem.".length) as ChemCommandType;
...
function buildRingEntity(command: ChemCommand): RingEntity {
  const kind = (command.props.kind ?? "").toLowerCase();
  const defaultSides = kind === "pentagon" ? 5 : 6;
  const sides = Math.max(
    3,
    Math.round(readNumber(command.props.sides) ?? defaultSides),
  );
  ...
  const order = normalizeBondOrder(
    command.props.order ?? command.props.bonds ?? (kind === "benzene" && !aromatic ? "alternating" : "1"),
    command.lineNumber,
  );
...
function emitCommand(
  command: ChemCommand,
  entities: ReadonlyMap<string, ChemEntity>,
  settings: Required<ChemMoleculePluginOptions>,
): string[] {
  switch (command.type) {
    case "atom":
      return emitAtom(entities.get(command.id) as AtomEntity, settings);
    case "bond":
      return emitBond(command, entities, settings);
    case "ring":
      return emitRing(entities.get(command.id) as RingEntity, settings);
    case "label":
...
function emitBond(
  ...
  const primitives = createBondPrimitives(
    command.id,
    trimmed.from,
    trimmed.to,
    order,
...
function emitRing(
  ...
  for (let index = 0; index < ring.vertices.length; index += 1) {
    ...
    const order = orderMode === "alternating" ? ((index % 2 === 0 ? 2 : 1) as 1 | 2) : orderMode;
    primitives.push(
...
function normalizeBondOrder(value: string, lineNumber: number): BondOrder {
...
function normalizeLabelSide(value: string | undefined): LabelSide {
  switch ((value ?? "top").toLowerCase()) {
    ...
    case "left":
    case "center":
      return (value?.toLowerCase() as LabelSide | undefined) ?? "top";
    default:
      return "top";
  }
}

function createBondPrimitives(
  baseId: string,
  from: Point2D,
  to: Point2D,
  order: 1 | 2 | 3,
  ...
  return offsets.map((offset, index) =>
    segmentPrimitive(
      helperId(baseId, `seg_${index + 1}`),
      shiftPoint(from, perp, offset),
      shiftPoint(to, perp, offset),
      style,
    ),
  );
}
...
function trimSegment(from: ResolvedRef, to: ResolvedRef): { from: Point2D; to: Point2D } {
...
function emitBareCommand(
  id: string,
  primitives: Primitive[],
  fallback: { x: number; y: number; items?: string[] },
): string[] {
  ...
  const output = primitives.map((primitive) =>
    emitPrimitive(primitive, bounds.minX, bounds.minY),
  );
  output.push(
    emitBareGroup(
      id,
...
function primitiveBounds(primitives: Primitive[]): {
  minX: number;
  minY: number;
  maxX: number;
  maxY: number;
...
function normalizeVector(point: Point2D): Point2D {
  const length = Math.hypot(point.x, point.y);
  if (length === 0) return { x: 0, y: -1 };
  return { x: point.x / length, y: point.y / length };
}

function shiftPoint(point: Point2D, direction: Point2D, amount: number): Point2D {
  return {
