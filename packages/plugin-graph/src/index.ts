...
export function compileGraph(source, options = {}) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (inTripleQuoteBlock || !trimmed.startsWith("graph.")) continue;
...
function resolveLayout(lines, autoAbsoluteLayout) {
  ...
  if (layoutIndex < 0) {
    if (!autoAbsoluteLayout) {
      throw new Error('Graph plugin requires "layout absolute"');
    }
    return { insertAfterDiagramIndex: diagramIndex };
  }

  if (lines[layoutIndex]?.trim() !== "layout absolute") {
    throw new Error('Graph commands require the root diagram to use "layout absolute"');
  }
...
function emitAxes(spec, settings) {
  ...
  if (spec.grid) {
    ...
    for (const value of xTicks) {
      if (almostEqual(value, yAxisValue)) continue;
      gridSegments.push([
    ...
    for (const value of yTicks) {
      if (almostEqual(value, xAxisValue)) continue;
      gridSegments.push([
    ...
    if (gridSegments.length) {
      output.push(
        serializeMultiPathNode(helperId(spec.id, "grid"), gridSegments, false, {
  ...
  if (spec.ticks) {
    ...
    for (const value of xTicks) {
      ...
      output.push(
        serializeNode("text", helperId(spec.id, `xtick_${safeNumberToken(value)}`), formatTickValue(value), {
          x: p.x - 18,
          y: p.y + 10,
          width: 36,
    ...
    for (const value of yTicks) {
      ...
      output.push(
        serializeNode("text", helperId(spec.id, `ytick_${safeNumberToken(value)}`), formatTickValue(value), {
          x: p.x - 50,
          y: p.y - 9,
          width: 42,
          "font-size": readNumber(spec.props["tick-font-size"]) ?? settings.tickFontSize,
    ...
    if (tickSegments.length) {
      output.push(
        serializeMultiPathNode(helperId(spec.id, "ticks"), tickSegments, false, {
  ...
  output.push(
    serializeMultiPathNode(spec.id, axisSegments, false, {
      stroke: spec.props.stroke ?? settings.axisStroke,
      "stroke-width": spec.props["stroke-width"] ?? String(settings.axisStrokeWidth),
      theme: spec.props.theme,
  ...
  output.push(
    serializeArrowHeadNode(
      helperId(spec.id, "x_tip"),
      xAxisEnd,
      { x: 1, y: 0 },
      {
        stroke: spec.props.stroke ?? settings.axisStroke,
  ...
  if (spec.props.label) {
    output.push(
      serializeNode("text", helperId(spec.id, "title"), spec.props.label, {
  ...
  if (spec.props.xlabel) {
    output.push(
      serializeNode("text", helperId(spec.id, "xlabel"), spec.props.xlabel, {
  ...
  if (spec.props.ylabel) {
    output.push(
      serializeNode("text", helperId(spec.id, "ylabel"), spec.props.ylabel, {
...
function emitPoint(spec, axesById, settings) {
  ...
  if (spec.label) {
    lines.push(
      serializeNode("text", labelNodeId(spec.id), spec.label, {
...
function emitRegion(command, axesById, pointsById, settings) {
  const axes = requireAxes(
    axesById,
    command.props.axes ?? inferAxesIdFromPointList(command.props.points, pointsById),
    command.id,
    "axes",
  );
...
function emitArea(command, axesById, plotsById, plotGeometryCache, settings) {
  ...
  const geometry = samplePlotGeometry(
    axes,
    target.fn,
    from,
    to,
    Math.max(16, Math.round(readNumber(command.props.samples) ?? target.samples ?? settings.samples)),
  );
...
function samplePlotGeometry(axes, fn, from, to, samples) {
  ...
  for (let index = 0; index <= samples; index += 1) {
    ...
    if (!Number.isFinite(y)) {
      if (currentSegment.length > 1) canvasSegments.push(currentSegment);
      currentSegment = [];
      continue;
    }
...
function resolveCoordinateRef(ref, axes, pointsById, lineNumber) {
  if (ref.kind === "coord") return ref.value;
...
function inferAxesIdFromPointList(value, pointsById) {
  if (!value || value[0] !== "[" || value[value.length - 1] !== "]") return undefined;
