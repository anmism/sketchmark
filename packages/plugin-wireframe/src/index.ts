...
type TextKind = "heading" | "body" | "caption" | "label";
type MediaKind = "image" | "avatar" | "icon";
type ControlKind = "button" | "input" | "textarea" | "select" | "checkbox" | "radio";
type ScreenFrame = "browser" | "phone" | "plain";
...
const SUPPORTED_COMMANDS = new Set<WireframeCommandType>([
  "screen",
  "panel",
  "text",
  "media",
...
const DEFAULTS: Required<WireframePluginOptions> = {
  surfaceFill: "#f7f0e3",
...
const TEXT_KIND_DEFAULTS: Record<
  TextKind,
  { width: number; lines: number; fontSize: number; fontWeight: number }
> = {
...
const TEXT_PLACEHOLDER_RATIOS: Record<TextKind, number[]> = {
  heading: [1, 0.72, 0.58],
...
export function compileWireframe(
  source: string,
  options: WireframePluginOptions = {},
): string {
  const settings = { ...DEFAULTS, ...options };
  const lines = source.split(/\r?\n/);
  const commandByLine = new Map<number, WireframeCommand>();
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (inTripleQuoteBlock || !trimmed.startsWith("wf.")) continue;

    commandByLine.set(index, parseWireframeCommand(trimmed, index + 1));
    hasWireframe = true;
  }

  if (!hasWireframe) return source;
...
function parseWireframeCommand(line: string, lineNumber: number): WireframeCommand {
  ...
  const type = commandToken.slice("wf.".length) as WireframeCommandType;
...
function emitCommand(
  command: WireframeCommand,
  settings: Required<WireframePluginOptions>,
): string[] {
  switch (command.type) {
    case "screen":
...
function emitScreen(
  ...
  if (command.props.label) appendProp(parts, "label", command.props.label);
...
function emitText(
  ...
  if (command.props.label) {
    ...
    appendProp(
      parts,
      "color",
      command.props.color ?? (kind === "caption" || kind === "label" ? settings.mutedTextColor : undefined),
    );
  ...
  const lineCount = Math.max(
    1,
    Math.round(readNumber(command.props.lines) ?? defaults.lines),
  );
...
function emitChoiceControl(
  ...
  const labelHeight = String(
    Math.max(
      readNumber(size) ?? 18,
      Math.round((readNumber(labelFontSize) ?? 14) * 1.2),
    ),
  );
...
function copyPropsToArray(
  source: Record<string, string>,
  target: string[],
  keys: readonly string[],
): void {
  for (const key of keys) {
    if (source[key] !== undefined) appendProp(target, key, source[key]);
  }
}

function appendKnownProps(
  parts: string[],
  props: Record<string, string>,
  keys: readonly string[],
): void {
  for (const key of keys) {
    if (props[key] !== undefined) appendProp(parts, key, props[key]);
  }
}
...
function compactChoiceLabelWidth(label: string, props: Record<string, string>): string {
...
function defaultScreenWidth(frame: ScreenFrame): number {
  switch (frame) {
    case "browser":
      return 960;
    case "phone":
...
function defaultScreenHeight(frame: ScreenFrame): number {
  switch (frame) {
    case "browser":
      return 640;
    case "phone":
...
function buildPlaceholderRatios(kind: TextKind, lineCount: number): number[] {
...
function controlLabel(
  command: WireframeCommand,
  kind: Exclude<ControlKind, "checkbox" | "radio">,
): string {
  if (kind === "button") return command.props.label ?? "Button";
  if (kind === "textarea") {
...
function controlWidth(
  ...
  if (command.props.width) return command.props.width;
  switch (kind) {
    case "button":
      return "120";
    case "input":
...
function controlHeight(
  ...
  if (command.props.height) return command.props.height;
  return kind === "textarea" ? "96" : "44";
}

function normalizeTextKind(value: string, lineNumber: number): TextKind {
...
function normalizeMediaKind(value: string, lineNumber: number): MediaKind {
...
function normalizeControlKind(value: string, lineNumber: number): ControlKind {
  if (
    value === "button" ||
    value === "input" ||
    value === "textarea" ||
...
function normalizeScreenFrame(value: string, lineNumber: number): ScreenFrame {
