...
type CircuitCommandType = "comp" | "port" | "junction" | "wire";
type ComponentKind =
  | "resistor"
  | "capacitor"
  | "inductor"
...
type CircuitEntity = ComponentSpec | PointEntity;

const SUPPORTED_COMMANDS = new Set<CircuitCommandType>([
  "comp",
  "port",
  "junction",
  "wire",
...
const SUPPORTED_COMPONENTS = new Set<ComponentKind>([
  "resistor",
  "capacitor",
  "inductor",
  "diode",
...
const DEFAULTS: Required<CircuitPluginOptions> = {
...
export function circuit(options: CircuitPluginOptions = {}): SketchmarkPlugin {
  return {
    name: "circuit",
    preprocess(source) {
...
export function compileCircuit(
  ...
  const commandByLine = new Map<number, CircuitCommand>();
  const entities = new Map<string, CircuitEntity>();
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (inTripleQuoteBlock || !trimmed.startsWith("ckt.")) continue;
...
function parseCircuitCommand(line: string, lineNumber: number): CircuitCommand {
  ...
  const type = commandToken.slice("ckt.".length) as CircuitCommandType;
...
function resolveLayout(
  ...
  if (!layoutMatch) {
    if (!autoAbsoluteLayout) {
      throw new Error('Circuit plugin requires "diagram layout=absolute"');
    }
  ...
  if (layoutMatch[1] !== "absolute") {
    throw new Error('Circuit commands require the root diagram to use "diagram layout=absolute"');
  }

  return { diagramIndex };
}

function buildComponentSpec(command: CircuitCommand): ComponentSpec {
...
function buildPointEntity(command: CircuitCommand): PointEntity {
...
function buildPins(
  ...
  const pins: Record<string, Point2D> = {
    left: { x: left, y },
    right: { x: left + w, y },
    top: { x, y: top },
    bottom: { x, y: top + h },
...
function emitCommand(
  ...
  entities: ReadonlyMap<string, CircuitEntity>,
  settings: Required<CircuitPluginOptions>,
): string[] {
  switch (command.type) {
    case "comp":
      return emitComponent(entities.get(command.id) as ComponentSpec, settings);
    case "port":
      return emitPort(entities.get(command.id) as PointEntity, settings);
    case "junction":
      return emitJunction(entities.get(command.id) as PointEntity, settings);
    case "wire":
...
function emitBareGroup(
  id: string,
  options: { x: number; y: number; width: number; height: number; items: string[] },
): string {
  const parts = ["bare", id];
...
function componentPath(kind: ComponentKind, orient: Orientation, w: number, h: number): string {
  switch (kind) {
    case "resistor":
    ...
    case "ground":
      return groundPath(w, h);
    case "switch":
      return orient === "h" ? switchPathH(w, h) : switchPathV(w, h);
  }
}

function resistorPathH(w: number, h: number): string {
  const cy = h / 2;
...
function resistorPathV(w: number, h: number): string {
  const cx = w / 2;
...
function capacitorPathH(w: number, h: number): string {
  const cy = h / 2;
...
function capacitorPathV(w: number, h: number): string {
  const cx = w / 2;
...
function inductorPathH(w: number, h: number): string {
  const cy = h / 2;
  const lead = 16;
  const bodyEnd = w - lead;
  const bodyStart = lead;
...
function inductorPathV(w: number, h: number): string {
  const cx = w / 2;
  const lead = 16;
  const bodyEnd = h - lead;
  const bodyStart = lead;
...
function diodePathH(w: number, h: number): string {
  const cy = h / 2;
  const start = 24;
  const bar = 60;
...
function diodePathV(w: number, h: number): string {
  const cx = w / 2;
  const start = 24;
  const bar = 60;
...
function sourcePathH(w: number, h: number): string {
  const cx = w / 2;
  const cy = h / 2;
...
function sourcePathV(w: number, h: number): string {
  const cx = w / 2;
  const cy = h / 2;
  const r = Math.max(10, Math.min(w, h) / 2 - 18);
  return [
...
function groundPath(w: number, h: number): string {
  const cx = w / 2;
  return [
    `M ${cx} 0 L ${cx} 8`,
    `M 2 8 L ${w - 2} 8`,
...
function switchPathH(w: number, h: number): string {
  const cy = h / 2;
  return `M 0 ${cy} L 34 ${cy} M 34 ${cy} L 76 2 M ${w - 34} ${cy} L ${w} ${cy}`;
}

function switchPathV(w: number, h: number): string {
  const cx = w / 2;
  return `M ${cx} 0 L ${cx} 34 M ${cx} 34 L 2 76 M ${cx} ${h - 34} L ${cx} ${h}`;
}

function resolveCircuitRef(
  ref: string,
  entities: ReadonlyMap<string, CircuitEntity>,
  lineNumber: number,
): Point2D {
...
function normalizeComponentKind(value: string | undefined, lineNumber: number): ComponentKind {
  if (!value) {
    throw new Error(`ckt.comp requires kind= on line ${lineNumber}`);
  }
  if (!SUPPORTED_COMPONENTS.has(value as ComponentKind)) {
    throw new Error(`Unsupported circuit component kind "${value}" on line ${lineNumber}`);
  }
  return value as ComponentKind;
}

function normalizeOrientation(value: string | undefined, kind: ComponentKind): Orientation {
  if (!value) return kind === "ground" ? "v" : "h";
  return value === "v" || value === "vertical" ? "v" : "h";
}

function normalizeWireMode(
  value: string,
  lineNumber: number,
): "auto" | "straight" | "hv" | "vh" {
...
function routeWire(
  ...
  if (resolvedMode === "straight") return [from, to];
  if (resolvedMode === "hv") return [from, { x: to.x, y: from.y }, to];
  return [from, { x: from.x, y: to.y }, to];
}

function wireBounds(points: Point2D[]): { minX: number; minY: number; maxX: number; maxY: number } {
  return {
...
function wireLabelPoint(points: Point2D[]): Point2D {
  if (points.length === 2) {
    return {
...
function portLabelPosition(side: string, r: number): { x: number; y: number; width: number } {
  switch (side) {
    case "left":
...
function appendProp(parts: string[], key: string, value?: string): void {
...
function requireNumber(props: Record<string, string>, key: string, lineNumber: number): number {
...
function helperId(baseId: string, suffix: string): string {
