...
const SUPPORTED_COMMANDS = new Set<GeometryCommandType>([
  "point",
  "segment",
  "ray",
  "line",
...
const DEFAULTS: Required<GeometryPluginOptions> = {
  pointRadius: 4,
  pointLabelDx: 12,
  pointLabelDy: -16,
  lineExtend: 80,
...
export function geometry(options: GeometryPluginOptions = {}): SketchmarkPlugin {
  return {
    name: "geometry",
    preprocess(source) {
...
export function compileGeometry(source: string, options: GeometryPluginOptions = {}): string {
  const settings = { ...DEFAULTS, ...options };
  const lines = source.split(/\r?\n/);
  const commandByLine = new Map<number, GeometryCommand>();
  const pointMap = new Map<string, PointSpec>();
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (inTripleQuoteBlock || !trimmed.startsWith("geo.")) continue;
...
function parseGeometryCommand(line: string, lineNumber: number): GeometryCommand {
  ...
  const type = commandToken.slice("geo.".length) as GeometryCommandType;
...
function buildPointSpec(
  ...
  settings: Required<GeometryPluginOptions>,
): PointSpec {
...
function emitCommand(
  command: GeometryCommand,
  pointMap: ReadonlyMap<string, PointSpec>,
  settings: Required<GeometryPluginOptions>,
): string[] {
  switch (command.type) {
    case "point":
...
function emitPoint(point: PointSpec): string[] {
  ...
  if (point.label) {
    lines.push(
      serializeNode("text", labelNodeId(point.id), point.label, {
...
function serializeArrowHeadNode(
  id: string,
  tip: Point2D,
  direction: Point2D,
  props: Record<string, string>,
...
function parseList(value: string | undefined, lineNumber: number, propName: string): string[] {
...
function labelNodeId(id: string): string {
  return `__geo_${sanitizeId(id)}_label`;
}

function arrowNodeId(id: string): string {
...
function polarPoint(center: Point2D, radius: number, deg: number): Point2D {
...
function sampleEllipsePoints(center: Point2D, rx: number, ry: number): Point2D[] {
  const segments = 36;
...
function normalizeArcClose(value: string | undefined, lineNumber: number): "none" | "chord" | "center" {
...
function lineLabelAnchor(start: Point2D, end: Point2D, offset: number): Point2D {
  const mid = {
    x: (start.x + end.x) / 2,
    y: (start.y + end.y) / 2,
  };
